...
class WindowManager {
    constructor() {
        ...
        this.boundHandlers = {
            ...
            windowClick: (event) => this.handleWindowClick(event),
    ...
    validateDOM() {
        if (!this.dom.windowManager) {
            throw new Error(
                'WindowManager: #window-manager no encontrado.'
            );
        }

        if (!this.dom.desktopArea) {
            throw new Error(
                'WindowManager: #desktop-area no encontrado.'
            );
        }
    }
    ...
    updateTaskbar() {
        ...
        minimizedWindows.forEach(
            (state) => {
                 ...
                 * La lógica de interacción continúa
                 * utilizando el listener delegado
                 * de la barra.
                 */
    ...
    handleTaskbarClick(event) {
        ...
        if (
            !button ||
            !this.dom.taskbar.contains(button)
        ) {
            return;
        }
    ...
    /*
     * pointerdown se utiliza únicamente para
     * enfocar la ventana.
     *
     * IMPORTANTE:
     * No se ejecutan aquí las acciones de
     * minimizar/maximizar/cerrar y no se utiliza
     * preventDefault(), ya que eso puede impedir
     * que los botones internos reciban su evento
     * click normal.
     */
    ...
    /*
     * Las acciones propias de la ventana se
     * procesan mediante click delegado.
     *
     * De esta manera los controles internos
     * de cada aplicación, como PIC-140,
     * conservan su propio ciclo pointerdown/click.
     */
